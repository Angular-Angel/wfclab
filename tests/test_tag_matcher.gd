class_name TagMatcherTest extends GdUnitTestSuite


func _solid(w: int, h: int, col: Color) -> Image:
...
func test_any_target_matches() -> void:
	var img := _solid(2, 2, Color("#ff0000"))
...
func test_transparent_pixels_excluded_from_both_sides() -> void:
	var img := _solid(2, 2, Color("#00ff00"))
	img.set_pixel(0, 0, Color(0, 0, 0, 0))
	assert_float(TagMatcher.coverage_fraction(img, ["#00ff00"], 0)).is_equal(1.0)
	var empty := _solid(2, 2, Color(0, 0, 0, 0))
